...
# Server Info
if config["backup"]["backup_server_info"]:
    backup_data["server"] = teamspeak.server_info(tn)

# Channels
if config["backup"]["backup_channels"]:
    backup_data["channels"] = teamspeak.channel_list(tn)

# Bans
if config["backup"]["backup_bans"]:
    backup_data["bans"] = teamspeak.ban_list(tn)

# Server Groups
if config["backup"]["backup_server_groups"]:
...
# Channel Groups
if config["backup"]["backup_channel_groups"]:
